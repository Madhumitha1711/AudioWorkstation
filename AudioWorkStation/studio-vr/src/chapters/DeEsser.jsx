...
// ── Types ────────────────────────────────────────────────────────────────────
// 0 = High-Pass/Shelf, 1 = Band-Pass — matches dsp-meta.json's Type menu exactly
...
async function renderDeesserOffline(generator, meta, dspModule, source, params, bypass) {
...
function normAndFade(buf, peakTarget = 0.55) {
    const chans = [];
...
function createDeesserDemoBuffer(ctx) {
    ...
    const fund = 130.8;
    const harmonics = [[1, 1.0], [2, 0.5], [3, 0.28], [4, 0.14], [5, 0.08]];
    ...
    for (const startS of burstStarts) {
        ...
        for (let i = 0; i < len && start + i < L.length; i++) {
            ...
            hpState.L = nL - hpState.L * 0.86;
            hpState.R = nR - hpState.R * 0.86;
...
function normalizeUploadedBuffer(buf, peakTarget = 0.85) {
    let peak = 0;
...
export default function DeEsser() {
    ...
    const startAudio = useCallback(async () => {
        ...
        if (!demoBufferRef.current)
            demoBufferRef.current = createDeesserDemoBuffer(ctx);
    ...
    const handleDownload = useCallback(async () => {
        ...
        setDownloading(true);
        try {
            const rendered = await renderDeesserOffline(generatorRef.current, dspMetaRef.current, dspModuleRef.current, track.buffer, params, bypass);
    ...
    const renderSourceRow = () => (<div className="eq-tabrow" style={{
      ...
      <button onClick={handleUploadClick} disabled={decoding} title="Upload your own audio to run through the de-esser" style={{
            display: 'flex', alignItems: 'center', gap: '0.35rem',
            padding: '0.3rem 0.65rem',
      ...
      {activeTrack && (<button onClick={() => { void handleDownload(); }} disabled={downloading} title="Render the active track through the de-esser and download it as a WAV" style={{
                display: 'flex', alignItems: 'center', gap: '0.35rem',
                padding: '0.3rem 0.65rem',
                background: 'var(--surface)',
                border: '1px dashed var(--border)',
      ...
      <div className="lab-topbar">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button className={`toggle-btn${isPlaying ? ' on' : ''}`} style={isPlaying ? { borderColor: 'var(--blue)', color: 'var(--blue)', background: 'var(--blue-dim)' } : {}} onClick={isPlaying ? stopAudio : () => { void startAudio(); }} disabled={!isPlaying && engineStatus !== 'ready'} title={engineStatus === 'loading' ? 'Loading Faust de-esser engine…' : engineStatus === 'error' ? (engineError ?? 'Faust engine failed to load') : undefined}>

...
function makeShaperCurve(type, driveDb) {
...
// ── Harmonic DFT ──────────────────────────────────────────────────────────────
function computeHarmonics(type, driveDb) {
...
function computeSourceSample(srcType, t) {
    switch (srcType) {
        case 'sine':
        ...
        case 'chord': return (Math.sin(t) * 0.5 +
            Math.sin(t * 329.63 / 261.63) * 0.35 +
            Math.sin(t * 392.00 / 261.63) * 0.25) * 0.78;
...
function drawTransferCurve(canvas, type, driveDb) {
    ...
    ctx.setLineDash([]);
    const curve = makeShaperCurve(type, driveDb);
    ctx.strokeStyle = 'rgba(255,122,69,0.06)';
    ctx.lineWidth = 12;
    ctx.beginPath();
    ...
    ctx.save();
    ctx.translate(12, H * 0.42);
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('↑ OUTPUT', 0, 0);
    ctx.restore();
}
// ── Canvas: static oscilloscope (math preview when paused) ────────────────────
function drawStaticOscilloscope(canvas, mode, srcType, satType, driveDb) {
    const hd = hiDpi(canvas);
    if (!hd)
        return;
    const { ctx, W, H } = hd;
...
function pickPreviewWindow(data, windowLen) {
    const candidates = 12;
    ...
    for (let start = 0; start + windowLen <= data.length; start += step) {
        let sum = 0;
        for (let i = 0; i < windowLen; i += 4)
            sum += data[start + i] * data[start + i];
        if (sum > bestEnergy) {
            bestEnergy = sum;
            bestStart = start;
        }
...
function drawStaticBufferPreview(canvas, mode, buffer, satType, driveDb) {
    const hd = hiDpi(canvas);
    if (!hd)
        return;
    const { ctx, W, H } = hd;
...
function knobDeg(value, min, max) {
...
function buildSource(ac, srcSel, uploadedTracks) {
    const nodes = [];
    ...
    const srcType = typeof srcSel === 'number' ? 'sine' : srcSel; // fallback if track missing
    if (srcType === 'chord') {
...
async function renderSaturatorOffline(source, satType, drive, tone, mix) {
...
function stopSourceNodes(nodes) {
...
// ── Component ──────────────────────────────────────────────────────────────────
export default function Saturator() {
    const [satType, setSatType] = useState('tape');
    ...
    const [srcType, setSrcType] = useState('sine');
    ...
    const [harmonics, setHarmonics] = useState(() => computeHarmonics('tape', 6.2));
    ...
    const dryWaveRef = useRef(null);
    const satWaveRef = useRef(null);
    ...
    // ── Task tracking ──────────────────────────────────────────────────────────
    useEffect(() => {
        if (satType !== 'digital' && drive <= 9)
            setTaskDriveOk(true);
    }, [drive, satType]);
    // Static-preview dispatcher: real buffer samples for an uploaded track,
    // the original math-based oscilloscope otherwise.
    const drawPreview = useCallback((canvas, mode, srcSel, sat, driveDb) => {
        if (!canvas)
            return;
        if (typeof srcSel === 'number') {
            const track = uploadedTracksRef.current.find(t => t.id === srcSel);
            if (track) {
                drawStaticBufferPreview(canvas, mode, track.buffer, sat, driveDb);
                return;
            }
        }
        drawStaticOscilloscope(canvas, mode, srcSel, sat, driveDb);
    }, []);
    // ── Canvas redraws ─────────────────────────────────────────────────────────
    useEffect(() => {
        if (transferRef.current)
            drawTransferCurve(transferRef.current, satType, drive);
    ...
    useEffect(() => {
        if (shaperRef.current)
            shaperRef.current.curve = makeShaperCurve(satType, drive);
    }, [satType, drive]);
    useEffect(() => {
    ...
    const animLoop = useCallback(() => {
        ...
        if (inAn && outAn) {
            ...
            if (dryWaveRef.current)
                drawLiveOscilloscope(dryWaveRef.current, inData, windowLen, 'dry', sat);
            if (satWaveRef.current)
                drawLiveOscilloscope(satWaveRef.current, outData, windowLen, 'sat', sat);
        }
    ...
    const handleSrcChange = useCallback((newSrc) => {
    ...
    return (<div className="sat-lab">
      ...
      <div className="sat-body">
        ...
        <div className="sat-left">
          ...
          <div className="sat-type-row">
            {SAT_TYPES.map(({ id, name, desc }) => (<div key={id} className={`sat-type-btn${satType === id ? ' active' : ''}`} onClick={() => setSatType(id)}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.4rem', marginBottom: '0.85rem' }}>
            {SRC_OPTIONS.map(({ id, label, desc }) => (<div key={id} onClick={() => handleSrcChange(id)} style={{
          ...
          <div className="eq-tabrow" style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '0.85rem' }}>
            ...
            return (<button key={track.id} onClick={() => handleSrcChange(track.id)} title={track.name} style={{
            ...
            <button onClick={handleUploadClick} disabled={decoding} title="Upload your own audio to run through the saturator" style={{
            display: 'flex', alignItems: 'center', gap: '0.35rem',
            padding: '0.3rem 0.65rem',
            ...
            {activeTrack && (<button onClick={() => { void handleDownload(); }} disabled={downloading} title="Render the active track through the saturator and download it as a WAV" style={{
                display: 'flex', alignItems: 'center', gap: '0.35rem',
                padding: '0.3rem 0.65rem',
                background: 'var(--surface)',
                border: '1px dashed var(--border)',
          ...
          <div className="curve-display">
            <canvas ref={transferRef} width={600} height={360} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}/>
          </div>
        ...
        <div className="sat-right">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem', marginBottom: '1.25rem' }}>
            <div className="knob-wrap">
              <div className="sat-knob" style={{ '--knob-rot': `${driveDeg.toFixed(1)}deg` }}/>
              <div className="knob-name">DRIVE</div>
              <div className="knob-val" style={{ color: 'var(--sat)' }}>{drive.toFixed(1)} dB</div>
              <input type="range" className="param-slider" min={0} max={12} step={0.1} value={drive} style={{ marginTop: '0.3rem' }} onChange={e => setDrive(Number(e.target.value))}/>
            </div>
            <div className="knob-wrap">
              <div className="sat-knob" style={{ '--knob-rot': `${toneDeg.toFixed(1)}deg` }}/>
              <div className="knob-name">TONE</div>
              <div className="knob-val" style={{ color: 'var(--sat)' }}>{toneLabel}</div>
              <input type="range" className="param-slider" min={0} max={100} step={1} value={tone} style={{ marginTop: '0.3rem' }} onChange={e => setTone(Number(e.target.value))}/>
            </div>
            <div className="knob-wrap">
              <div className="sat-knob" style={{ '--knob-rot': `${mixDeg.toFixed(1)}deg` }}/>
              <div className="knob-name">MIX</div>
              <div className="knob-val" style={{ color: 'var(--sat)' }}>{mix.toFixed(0)}%</div>
              <input type="range" className="param-slider" min={0} max={100} step={1} value={mix} style={{ marginTop: '0.3rem' }} onChange={e => setMix(Number(e.target.value))}/>
            </div>
          </div>
          ...
          <div className="harmonics-display">
            <div className="harmonics-bars">
              {harmonics.map((h, i) => (<div key={i} className="harmonic-bar-wrap">
          ...
          <div className="waveform-compare">
            ...
            <div className="compare-row">
              ...
              <div className="mini-wave" style={{ height: 68, borderColor: 'rgba(255,122,69,0.25)', background: '#0D0D0F' }}>
                <canvas ref={satWaveRef} width={600} height={140} style={{ width: '100%', height: '100%', display: 'block' }}/>
              </div>
            </div>
          </div>
      ...
      <div className="lab-footer">
        <div className="task-list" style={{ flexDirection: 'row', gap: '1rem' }}>
          ...
          <div className="task-item">
            <div className={`task-check${taskIdentified ? ' done' : ''}`} style={{ cursor: 'pointer' }} title="Click to mark complete" onClick={() => setTaskIdentified(true)}>
              {taskIdentified ? '✓' : ''}
            </div>

...
export interface KnobSpec {
  ...
  step:  number;
  fmt:   (v: number) => string;
  accent?: string; // CSS color e.g. 'var(--teal)'
}
...
export function Knob({ spec, value, onChange, disabled = false, target, size = 64 }: {
  ...
  disabled?: boolean;
  /** Optional reference value — draws a small amber tick on the dial (e.g. a target or unity point). */
  target?: number;
  /** Knob diameter in px. Defaults to 64, matching the original Chapter 6 dial. */
  size?: number;
}) {
    ...
    <div className="knob-wrap" style={disabled ? { opacity: 0.35, pointerEvents: 'none' } : {}}>
      <div style={{ position: 'relative', width: size, height: size }}>
        <svg style={{ position: 'absolute', top: 0, left: 0 }} width={size} height={size} viewBox={`${-size / 2} ${-size / 2} ${size} ${size}`}>
          ...
          {targetPt && (
            <line
              x1={targetPt.a.x} y1={targetPt.a.y}
              x2={targetPt.b.x} y2={targetPt.b.y}

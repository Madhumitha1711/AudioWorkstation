...
export interface FaderSpec {
  ...
  step:  number;
  fmt:   (v: number) => string;
  accent?: string; // CSS color e.g. 'var(--amber)'
}
...
export function Fader({ spec, value, onChange, disabled = false, target, height = 90 }: {
  ...
  disabled?: boolean;
  /** Optional reference value — draws a small amber tick on the track (e.g. a target or unity point). */
  target?: number;
  /** Track height in px. Defaults to 90, matching the channel level-meter strip. */
  height?: number;
}) {

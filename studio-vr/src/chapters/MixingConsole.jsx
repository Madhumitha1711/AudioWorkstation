...
function volKnobValue(pos) { return 1 - pos; }
function volKnobToPos(v) { return Math.max(0, Math.min(0.99, 1 - v)); }
...
function addSine(d, sr, t0, freq, amp, dur) {
...
function addNoise(d, sr, t0, amp, dur, decay) {
    const s0 = Math.floor(t0 * sr);
    const sN = Math.min(d.length, s0 + Math.floor(dur * sr));
    for (let i = s0; i < sN; i++) {
        const t = (i - s0) / sr;
...
function makeBass(ctx) {
    ...
    for (let i = 2; i < d.length; i++)
        d[i] = (d[i] + d[i - 1] + d[i - 2]) / 3;
    normalize(d, 0.82);
    return buf;
}
// LEAD VOX: G4-B4-D5 melodic phrase
function makeVox(ctx) {
    const sr = ctx.sampleRate;
    const buf = ctx.createBuffer(1, sr * LOOP_DUR, sr);
    const d = buf.getChannelData(0);
    ...
    for (let b = 0; b < 8; b++) {
        ...
        for (let i = s0; i < sN; i++) {
            ...
            for (let h = 1; h <= maxH; h++)
                sig += (1 / Math.pow(h, 1.3)) * Math.sin(ph * h);
            // slight vibrato
    ...
    for (const t0 of [0.25, 0.75, BEAT * 4 + 0.25, BEAT * 4 + 0.75]) {
        for (const [si, freq] of strings.entries()) {
            ...
            for (let i = s0; i < sN; i++) {
                ...
                for (let h = 1; h <= maxH; h++)
                    sig += (1 / Math.pow(h, 1.9)) * Math.sin(ph * h);
                d[i] += env * 0.32 * sig;
                ph += (2 * Math.PI * freq) / sr;
            }
        }
...
export default function MixingConsole() {
    ...
    const [masterFill, setMasterFill] = useState([0.72, 0.65]);
    const [trackFill, setTrackFill] = useState(TRACKS.map(() => 0));
    ...
    const bufsRef = useRef(Array(6).fill(null));
    const gainRefs = useRef(Array(6).fill(null));
    const panRefs = useRef(Array(6).fill(null));
    const anlRefs = useRef(Array(6).fill(null));
    const srcRefs = useRef(Array(6).fill(null));
    const masterGRef = useRef(null);
    const masterARef = useRef(null);
    ...
    // Apply master gain
    useEffect(() => {
        if (masterGRef.current)
            masterGRef.current.gain.value = posToLinear(masterPos);
    }, [masterPos]);
    // ── Task checking ─────────────────────────────────────────────────────────────
    useEffect(() => {
        ...
        TRACKS.forEach((_, i) => {
            const eff = muted[i] || (anySolo && !soloed[i]);
            if (!eff)
                rmsEst += Math.pow(posToLinear(faderPos[i]) * weights[i], 2);
        });
    ...
    const buildGraph = useCallback(async () => {
        ...
        for (let i = 0; i < TRACKS.length; i++) {
            ...
            if (!bufsRef.current[i])
                bufsRef.current[i] = MAKERS[i](ctx);
            // Nodes
    ...
    const renderMixOffline = useCallback(async () => {
    ...
    return (<div className="mix-lab">
      ...
      <div className="mix-body">
        ...
        <div className="tracks-grid">
          {TRACKS.map((trk, i) => {
            ...
            return (<div key={trk.id} className={`channel-strip${isSelected ? ' selected' : ''}`} style={isSelected ? { borderColor: trk.color, boxShadow: `0 0 12px ${trk.color}22` } : {}} onClick={() => setSelTrack(i)}>
                {/* Name */}
                ...
                <div className="channel-btns">
                  <div className={`ch-btn${muted[i] ? ' m-active' : ''}`} onClick={e => { e.stopPropagation(); toggleMute(i); }}>M</div>
                  <div className={`ch-btn${soloed[i] ? ' s-active' : ''}`} onClick={e => { e.stopPropagation(); toggleSolo(i); }}>S</div>
                </div>
                ...
                <Knob size={44} spec={{
                    ...
                    fmt: v => panLabel(v),
                    accent: 'var(--amber)',
                }} value={panValues[i]} onChange={v => setPanValues(prev => { const n = [...prev]; n[i] = v; return n; })}/>
                ...
                }} value={volKnobValue(faderPos[i])} target={volKnobValue(UNITY_POS)} onChange={v => setFaderPos(prev => { const n = [...prev]; n[i] = volKnobToPos(v); return n; })}/>

                {/* Level meter */}
                <div className="level-meter-strip">
                  {/* Left column */}
                  <div className="level-col">
                    {segsL.map((cls, si) => cls ? <div key={si} className={`level-seg ${cls}`} style={{ height: '3px' }}/> : null)}
                  </div>
                  {/* Right column */}
                  <div className="level-col">
                    {segsR.map((cls, si) => cls ? <div key={si} className={`level-seg ${cls}`} style={{ height: '3px' }}/> : null)}
                  </div>
                </div>
              </div>);
        })}
          ...
          <div className="master-strip">
            ...
            <div className="master-meters">
              {[masterFill[0], masterFill[1]].map((fill, ci) => (<div key={ci} className="master-col">
                  <div className="master-col-fill" style={{
                height: `${Math.max(2, fill * 100)}%`,
                background: masterBarH(fill),
            }}/>
                </div>))}
            </div>
            <div className="loudness-label" style={{ color: taskDone[2] ? 'var(--green)' : 'var(--text-faint)' }}>
              {lufsStr} LUFS
            </div>
            {/* Master gain knob */}
            <Knob size={52} spec={{
            ...
            accent: 'var(--amber)',
        }} value={volKnobValue(masterPos)} target={volKnobValue(UNITY_POS)} onChange={v => setMasterPos(volKnobToPos(v))}/>
          </div>
        </div>
      ...
      <div className="lab-footer">
        ...
        ].map((label, i) => (<div key={i} className="task-item">
        ...
        <div className="btn-row">
          ...
          {downloadError && (<span style={{ fontFamily: 'var(--mono)', fontSize: '0.6rem', color: 'var(--red)' }}>
              {downloadError}
            </span>)}
          <button className="btn-secondary" onClick={isPlaying ? handleStop : handleReset}>
            {isPlaying ? '⏹ Stop' : '↺ Reset'}
          </button>
          <button className="btn-secondary" onClick={handlePlay} style={isPlaying ? { borderColor: 'var(--green)', color: 'var(--green)' } : {}}>
            {isPlaying ? '⏸ Playing' : '▶ Play Session'}
          </button>
          <button className="btn-secondary" onClick={() => { void handleDownload(); }} disabled={downloading} title="Render the current mix (fader, pan, mute/solo, master gain) and download it as a WAV">
            {downloading ? '⏳ Rendering…' : '⬇ Download Mix'}
          </button>
          <button className="btn-primary" onClick={() => setSubmitted(true)} disabled={submitted && taskDone.every(Boolean)}>

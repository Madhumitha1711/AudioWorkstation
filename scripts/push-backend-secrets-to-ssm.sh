#!/bin/bash
# Reads studio-backend/.env (your real local values) and pushes exactly the
# keys studio-backend-task-def.json expects under "secrets" into SSM
# Parameter Store as SecureString, under /studio/dev/*.
#
# Usage: ./push-backend-secrets-to-ssm.sh [path/to/studio-backend/.env]
# Defaults to ../studio-backend/.env (i.e. run from the scripts/ folder).
#
# Safe to re-run — existing parameters are overwritten in place.
# Empty values in .env are skipped with a warning instead of pushing a
# blank secret (which would silently disable that feature at runtime).
#
...
ENV_FILE="${1:-$(dirname "$0")/../studio-backend/.env}"
PREFIX="/studio/dev"
REGION="ap-south-1"
...
# local .env var : SSM parameter name (differs only where a name is
# ambiguous across services, e.g. AWS_ACCESS_KEY_ID also exists in
# studio-cms/.env with a possibly different value).
MAP=(
  ...
  "CLOUDFLARE_STREAM_KEY_ID:CLOUDFLARE_STREAM_KEY_ID"
  "CLOUDFLARE_STREAM_SIGNING_KEY:CLOUDFLARE_STREAM_SIGNING_KEY"
